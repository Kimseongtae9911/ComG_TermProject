...
HRESULT CStage2::Initialize()
{
	...
	for (int i = 1; i < 6; ++i) {
		...
		if (!BlockObj)
		{
			BlockObj = CObject::Create("../Resource/MapCube/cube3.obj", glm::vec3(-15 + 1.0f * 5, 1.0f * i - 1.0f, 0.0f), { 0.6, 0.6, 0.6, 1.0 });
		}
		else
		{
			BlockObj = CObject::Create(dynamic_cast<CObject*>(BlockObj), "../Resource/MapCube/cube3.obj", glm::vec3(-15 + 1.0f * 5, 1.0f * i - 1.0f, 0.0f), { 0.6, 0.6, 0.6, 1.0 });
		}
		dynamic_cast<CObject*>(BlockObj)->Set_OBJID(OBJ_ID::OBJ_MAP);
		if (FAILED(m_pGameMgr->Add_GameObj(OBJ_ID::OBJ_MAP, BlockObj)))
			return E_FAIL;
	...
	for (int i = 1; i < 10; ++i) {
		...
		if (!BlockObj)
		{
			BlockObj = CObject::Create("../Resource/MapCube/cube3.obj", glm::vec3(-15 + 1.0f * 15, 1.0f * i - 1.0f, 0.0f), { 0.6, 0.6, 0.6, 1.0 });
		}
		else
		{
			BlockObj = CObject::Create(dynamic_cast<CObject*>(BlockObj), "../Resource/MapCube/cube3.obj", glm::vec3(-15 + 1.0f * 15, 1.0f * i - 1.0f, 0.0f), { 0.6, 0.6, 0.6, 1.0 });
		}
		dynamic_cast<CObject*>(BlockObj)->Set_OBJID(OBJ_ID::OBJ_MAP);
		if (FAILED(m_pGameMgr->Add_GameObj(OBJ_ID::OBJ_MAP, BlockObj)))
			return E_FAIL;
