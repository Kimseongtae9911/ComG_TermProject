...
HRESULT CStage1::Initialize()
{
	...
	for (int i = 0; i < 15; ++i)
	{
		for (int j = 0; j < 30; ++j)
		{
			if (i == 0 || i == 14)
			{
				pObj = CObject::Create("../Resource/MapCube/Cube.obj", glm::vec3(-15 + 1.0f * j, 1.0f * i - 0.5f, 0.0f), { 0.5, 0.5, 0.5, 1.0 });
	...
	pObj = CObject::Create("../Resource/MapCube/Cube.obj", glm::vec3(-15 + 1.0f * 28, 1.0f * 10 - 0.5f, 0.0f), { 0.6, 0.6, 1.0, 1.0 });
	if (FAILED(m_pGameMgr->Add_GameObj(OBJ_MAP, pObj)))
		return E_FAIL;
	pObj = CObject::Create("../Resource/MapCube/Cube.obj", glm::vec3(-15 + 1.0f * 27, 1.0f * 10 - 0.5f, 0.0f), { 0.6, 0.6, 1.0, 1.0 });
	if (FAILED(m_pGameMgr->Add_GameObj(OBJ_MAP, pObj)))
		return E_FAIL;
	pObj = CObject::Create("../Resource/MapCube/Cube.obj", glm::vec3(-15 + 1.0f * 26, 1.0f * 10 - 0.5f, 0.0f), { 0.6, 0.6, 1.0, 1.0 });
	if (FAILED(m_pGameMgr->Add_GameObj(OBJ_MAP, pObj)))
		return E_FAIL;
